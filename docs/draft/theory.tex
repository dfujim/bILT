...
Fortunately, this convolution with the beam can be solved analytically. For probes arriving at time $t'$ during a beam pulse of duration $\Delta$ and decaying at later time $t$, the time dependence of $P(t)$ written as~\cite{2006-Salman-PRL-96-147601, 2015-MacFarlane-PRB-92-064409}
%
\begin{equation} \label{eq:polarization}
P(t) = P_0 \frac{ \displaystyle \int_{0}^{\eta} \exp \left [ -\left (\eta - t^{\prime} \right ) / \tau_{\beta} \right ] \, R \left (t,t^{\prime} \right ) \, \mathrm{d}t^{\prime} }{ \displaystyle \int_{0}^{\eta} \exp \left [ -t^{\prime} / \tau_{\beta} \right ] \, \mathrm{d}t^{\prime} },
%   P(t) = P_{0} \times
%   \begin{cases}
...
\end{equation}
%
where $\eta = t$ if $t\leq \Delta$, and $\eta = \Delta$ otherwise. $P_{0}$ is the degree of polarization at $t = 0$ (determined by the optical pumping of \ch{^{8}Li} prior to implantation), and $R \left (t, t^{\prime} \right )$ is the relaxation function.
...
This form of $R \left ( t, t^{\prime} \right )$ can arise when the \gls{kww} distribution is chosen for $p(\lambda)$~\cite{2006-Johnston-PRB-74-184430}. The special cases of $\beta = 1/2$~\cite{1968-Tse-PRL-21-511, 1984-Stockmann-JNCS-66-501} or $\beta = 1/3$~\cite{1992-Bader-JPCM-4-4779} originate from \gls{slr} that is inhomogenously averaged in \gls{3d} or \gls{2d} limits.
...
While the \gls{kww} distribution is successful in describing a great deal of relaxation phenomena, it is hardly the simplest distribution to work with, and may in fact be too successful. The high degree of correlation between $\lambda$ and $\beta$, allows stretched exponential to do a good job in fitting relaxation spectra whose underlying may be much simpler (e.g. a biexponential). This ease of use of the stretched exponential may disguise the proper relaxation function and lead to an incorrect interpretation of the relaxation mechanism. We now describe how the \gls{ilt} may be used to determine $p ( \lambda )$, and reveal the underlying distribution of exponentials. 
